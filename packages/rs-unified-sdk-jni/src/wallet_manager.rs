...
/// success.
fn pwffi_error_message(mut result: platform_wallet_ffi::PlatformWalletFFIResult) -> Option<String> {
...
/// Per-wallet Core SPV state snapshot — the read half of the rescan
/// diagnostics the host-side backfill gate currently has to infer. Wraps
/// `platform_wallet_core_wallet_state`.
///
/// Returns a `long[3]` = `{ syncedHeight, lastProcessedHeight,
/// monitorRevision }`. `syncedHeight` is the filter-scan checkpoint (the
/// field `spvRescanFilters` rewinds and the pipeline advances);
/// `lastProcessedHeight` is the monotonic processed watermark;
/// `monitorRevision` is the wallet's monitored-script-set revision.
/// Throws on a bad handle or unknown wallet.
#[no_mangle]
pub extern "system" fn Java_org_dashfoundation_dashsdk_ffi_WalletManagerNative_coreWalletState(
    mut env: JNIEnv,
    _class: JClass,
    manager_handle: jlong,
    wallet_id: JByteArray,
    ...
    guard(&mut env, std::ptr::null_mut(), |env| {
        ...
        let mut state = platform_wallet_ffi::core_wallet_types::CoreWalletStateFFI {
            synced_height: 0,
        ...
        let values = [
            state.synced_height as jlong,
            state.last_processed_height as jlong,
            state.monitor_revision as jlong,
        ];
        match env.new_long_array(3) {
            Ok(array) => {
                if env.set_long_array_region(&array, 0, &values).is_err() {
                    let _ = env.exception_clear();
                    throw_sdk_exception(env, 99, "coreWalletState array write failed");
                    return std::ptr::null_mut();
                }
                array.into_raw()
            }
            Err(_) => {
                let _ = env.exception_clear();
                throw_sdk_exception(env, 99, "coreWalletState array allocation failed");
                std::ptr::null_mut()
            }
        }
    })
...
/// One account's spent-outpoint inventory — the second half of the
/// store-reconcile surface (`walletManagerUtxosPageJson` is the unspent
/// half). Wraps `platform_wallet_account_spent_outpoints`.
///
/// The account is addressed by its `AccountType` fields rather than a full
/// `AccountSpec`: `account_xpub_bytes` is not consulted for the lookup, so
/// only the discriminant tags and indices are passed. `typeTag` is the
/// `AccountTypeTagFFI` byte (0 Standard, 1 CoinJoin, 12 DashpayReceiving,
/// 13 DashpayExternal, …); `standardTag` the `StandardAccountTypeTagFFI`
/// byte (0 BIP44, 1 BIP32) used only when `typeTag == 0`; `index`,
/// `registrationIndex`, `keyClass`, and the two 32-byte identity ids fill
/// the fields the selected account type needs (unused ones are ignored).
///
/// Returns a flat big-endian blob, 36 bytes per outpoint: 32-byte txid
/// (raw internal byte order, matching `walletManagerUtxosPageJson`'s
/// hex→bytes) + u32 vout. An empty inventory returns a zero-length array.
/// Throws on a bad handle, unknown wallet, or an account spec that does
/// not map to a key-wallet account type.
#[no_mangle]
pub extern "system" fn Java_org_dashfoundation_dashsdk_ffi_WalletManagerNative_accountSpentOutpoints(
    mut env: JNIEnv,
    _class: JClass,
    manager_handle: jlong,
    wallet_id: JByteArray,
    ...
    standard_tag: jint,
    index: jint,
    registration_index: jint,
    key_class: jint,
    user_identity_id: JByteArray,
    friend_identity_id: JByteArray,
) -> jbyteArray {
    guard(&mut env, std::ptr::null_mut(), |env| {
        let Some(wallet_id) = read_id32(env, &wallet_id) else {
            return std::ptr::null_mut();
        };
        if !(0..=255).contains(&type_tag) || !(0..=255).contains(&standard_tag) {
            throw_sdk_exception(env, 1, "typeTag / standardTag must be a byte value");
            return std::ptr::null_mut();
        }
        // The identity ids matter only for the DashPay account types; for
        // everything else a 32-zero default is fine and unread.
        let user_id = read_id32_or_zero(env, &user_identity_id);
        let friend_id = read_id32_or_zero(env, &friend_identity_id);
        let spec = platform_wallet_ffi::wallet_restore_types::AccountSpecFFI {
            type_tag: type_tag as u8,
            standard_tag: standard_tag as u8,
            index: index as u32,
            registration_index: registration_index as u32,
            key_class: key_class as u32,
            user_identity_id: user_id,
            friend_identity_id: friend_id,
            account_xpub_bytes: std::ptr::null(),
            account_xpub_bytes_len: 0,
        };
        let mut entries: *const platform_wallet_ffi::core_wallet_types::OutPointFFI =
            std::ptr::null();
        let mut count: usize = 0;
        let result = unsafe {
        ...
        struct SpentGuard {
            entries: *const platform_wallet_ffi::core_wallet_types::OutPointFFI,
            count: usize,
        }
        ...
        let _free = SpentGuard { entries, count };
        if take_pwffi_error(env, result) {
            return std::ptr::null_mut();
        }
        if count > (i32::MAX as usize) / 36 {
            throw_sdk_exception(env, 99, "spent-outpoint inventory too large");
            return std::ptr::null_mut();
        }
        let mut blob = vec![0u8; count * 36];
        if count > 0 && !entries.is_null() {
            let rows = unsafe { std::slice::from_raw_parts(entries, count) };
            for (i, op) in rows.iter().enumerate() {
                let off = i * 36;
                blob[off..off + 32].copy_from_slice(&op.txid);
                blob[off + 32..off + 36].copy_from_slice(&op.vout.to_be_bytes());
            }
        }
        match env.byte_array_from_slice(&blob) {
            Ok(array) => array.into_raw(),
            Err(_) => {
                let _ = env.exception_clear();
                throw_sdk_exception(env, 99, "spent-outpoint blob allocation failed");
                std::ptr::null_mut()
            }
        }
    })
...
/// Read a 32-byte id, or return all-zeroes when the array is null/absent —
/// used for the optional DashPay identity ids on `accountSpentOutpoints`.
fn read_id32_or_zero(env: &mut JNIEnv, arr: &JByteArray) -> [u8; 32] {
    if arr.is_null() {
        return [0u8; 32];
    }
    read_id32(env, arr).unwrap_or([0u8; 32])
}
