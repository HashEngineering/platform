...
pub enum PlatformWalletFFIResultCode {
    ...
    ///
    /// Code 49 — renumbered from the branch's original 48 when merged
    /// #4356 took 47 and 48 (`ErrorAssetLockInputConflict` /
    /// [`Self::ErrorAssetLockInputContested`]) as ABI; merged ABI wins and
    /// the open PR renumbers via the frontier. See ERROR_CODE_REGISTRY.md.
    ErrorShieldedClaimUnconfirmed = 49,
...
mod tests {
    ...
    /// This PR's fourth shielded-invite code, pinned like 43-45 above. 49 is
    /// from the registry frontier — renumbered from the branch's original 48
    /// when merged #4356 took 47/48 as ABI (merged ABI wins; the open PR
    /// renumbers via the frontier). Moving it again would silently
    /// reclassify an ambiguous claim outcome on every host built against
    /// the shipped numbering.
    #[test]
    fn shielded_claim_unconfirmed_code_is_pinned_at_49() {
        assert_eq!(
