    ...
    async fn wait_for_chain_lock(
        ...
        loop {
            ...
            match deadline {
                ...
                //
                // NOT bounded by `LOCK_STATE_RE_POLL`. That bound exists for
                // the deadline arm, where a missed wake costs the whole
                // InstantSend-preference window; here there is no window to
                // lose, and `retry_tick` already re-reads the record every
                // `LOCATE_RETRY_INTERVAL`. Adding a one-second wake on this
                // arm only defeats that throttle — it doubles the lookups an
                // unmined transaction performs while it waits.
                None => {
                    tokio::select! {
                        _ = &mut notified => {}
                        _ = retry_tick => {}
