...
impl FileBackedShieldedStore {
    ...
    pub fn open_path(
        ...
        pending_conn
            ...
            .map_err(|e| {
                FileShieldedStoreError::Storage(format!("create lifecycle_admission table: {e}"))
            })?;
        ...
        pending_conn
            ...
            .map_err(|e| {
                FileShieldedStoreError::Storage(format!("create one_time_claim_reservation table: {e}"))
            })?;
        ...
        pending_conn
            ...
            .map_err(|e| FileShieldedStoreError::Storage(format!("create wallet_removal table: {e}")))?;
        let mut store = Self {
            tree: Mutex::new(tree),
            path,
            max_checkpoints,
    ...
    fn add_pending_spends_identity_index(
        ...
        let tx = conn
            ...
            .map_err(|e| FileShieldedStoreError::Storage(format!("begin pending_spends migration: {e}")))?;
        let present = {
            let mut stmt = tx
                .prepare("SELECT 1 FROM pragma_table_info('shielded_pending_spends') WHERE name = 'identity_index'")
                .map_err(|e| {
                    FileShieldedStoreError::Storage(format!("prepare pending_spends column probe: {e}"))
                })?;
            stmt.exists([])
                .map_err(|e| FileShieldedStoreError::Storage(format!("probe pending_spends columns: {e}")))?
        };
        if !present {
            match tx.execute(
                ...
                Err(e) => {
                    return Err(FileShieldedStoreError::Storage(format!(
        ...
        tx.commit()
            .map_err(|e| FileShieldedStoreError::Storage(format!("commit pending_spends migration: {e}")))?;
        Ok(())
    }
    ...
    fn lock_pending_conn(&self) -> Result<MutexGuard<'_, Connection>, FileShieldedStoreError> {
        self.pending_conn
            .lock()
            .map_err(|e| FileShieldedStoreError::Storage(format!("pending_conn mutex poisoned: {e}")))
    }
    ...
    pub(crate) fn has_pending_row_for_tests(
        ...
        let count: i64 = conn
            ...
            .map_err(|e| FileShieldedStoreError::Storage(format!("count pending rows: {e}")))?;
        Ok(count > 0)
    }
    ...
    fn rehydrate_pending_spends(&mut self) -> Result<(), FileShieldedStoreError> {
        ...
        let raw_rows: Vec<_> = {
            ...
            let mut stmt = conn
                .prepare(
                    "SELECT wallet_id, account_index, activity_id, anchor, nullifiers, st_bytes, \
                     attempts, identity_index, identity_nonce_finalized, \
                     identity_user_abandoned FROM shielded_pending_spends",
                )
            ...
            rows.collect::<Result<Vec<_>, _>>()
                .map_err(|e| FileShieldedStoreError::Storage(format!("read rehydrate row: {e}")))?
        };
        for row in raw_rows {
            // `raw_rows` was collected with `collect::<Result<Vec<_>, _>>()?`, so the
            // per-row rusqlite error is already handled; each item is the pair the
            // query closure returned.
            let (classifiable_st_bytes, decoded) = row;
    ...
    fn read_pending_row(
        ...
        let row = conn
            ...
            .optional()
            .map_err(|e| FileShieldedStoreError::Storage(format!("read pending claim record: {e}")))?;
    ...
    fn reap_expired_admissions(
        ...
        .map_err(|e| FileShieldedStoreError::Storage(format!("reap expired admissions: {e}")))?;
        Ok(())
    }
...
impl ShieldedStore for FileBackedShieldedStore {
            ...
            conn.execute(
                "INSERT OR REPLACE INTO shielded_pending_spends \
                 (wallet_id, account_index, activity_id, anchor, nullifiers, st_bytes, attempts, \
                  identity_nonce_finalized, identity_user_abandoned, identity_index) \
    ...
    fn purge_wallet(&mut self, wallet_id: WalletId) -> Result<(), Self::Error> {
        ...
        {
            return Err(FileShieldedStoreError::Storage(
    ...
    fn record_wallet_removal(&mut self, wallet_id: WalletId) -> Result<(), Self::Error> {
        ...
        .map_err(|e| FileShieldedStoreError::Storage(format!("record wallet removal tombstone: {e}")))?;
        Ok(())
    }
    ...
    fn clear_wallet_removal(&mut self, wallet_id: WalletId) -> Result<(), Self::Error> {
        ...
        .map_err(|e| FileShieldedStoreError::Storage(format!("clear wallet removal tombstone: {e}")))?;
        Ok(())
    }
    ...
    fn outstanding_wallet_removals(&self) -> Result<Vec<WalletId>, Self::Error> {
        ...
        let mut stmt = conn
            ...
            .map_err(|e| {
                FileShieldedStoreError::Storage(format!("prepare wallet removal tombstone read: {e}"))
            })?;
        let rows = stmt
            .query_map([], |row| row.get::<_, Vec<u8>>(0))
            .map_err(|e| FileShieldedStoreError::Storage(format!("read wallet removal tombstones: {e}")))?;
        let mut out = Vec::new();
        for row in rows {
            let raw =
                row.map_err(|e| FileShieldedStoreError::Storage(format!("wallet removal row: {e}")))?;
    ...
    fn begin_claim_admission(
        ...
        let tx = conn
            .transaction_with_behavior(rusqlite::TransactionBehavior::Immediate)
            .map_err(|e| FileShieldedStoreError::Storage(format!("begin claim admission: {e}")))?;
        ...
        let blocked: i64 = tx
            ...
            .map_err(|e| FileShieldedStoreError::Storage(format!("read destructive barriers: {e}")))?;
        if blocked > 0 {
        ...
        .map_err(|e| FileShieldedStoreError::Storage(format!("insert claim lease: {e}")))?;
        tx.commit()
            .map_err(|e| FileShieldedStoreError::Storage(format!("commit claim admission: {e}")))?;
        Ok(true)
    }
    ...
    fn renew_claim_admission(
        ...
        let tx = conn
            .transaction_with_behavior(rusqlite::TransactionBehavior::Immediate)
            .map_err(|e| FileShieldedStoreError::Storage(format!("begin claim lease renewal: {e}")))?;
        let updated = tx
            .execute(
            ...
            .map_err(|e| FileShieldedStoreError::Storage(format!("renew claim lease: {e}")))?;
        if updated > 0 {
            ...
            .map_err(|e| FileShieldedStoreError::Storage(format!("renew claim-key reservation: {e}")))?;
        }
        tx.commit()
            .map_err(|e| FileShieldedStoreError::Storage(format!("commit claim lease renewal: {e}")))?;
        Ok(updated > 0)
    }
    ...
    fn reserve_one_time_claim_key(
        ...
        let (reservation, pending) = {
            ...
            let tx = conn
                .transaction_with_behavior(rusqlite::TransactionBehavior::Immediate)
                .map_err(|e| FileShieldedStoreError::Storage(format!("begin claim-key reservation: {e}")))?;
            ...
            .map_err(|e| FileShieldedStoreError::Storage(format!("reap claim-key reservations: {e}")))?;
            ...
            let inserted = tx
                ...
                .map_err(|e| {
                    FileShieldedStoreError::Storage(format!("insert claim-key reservation: {e}"))
                })?;
            let reservation = if inserted > 0 {
                ...
                let (holder, expires_at): (Vec<u8>, i64) = tx
                    ...
                    .map_err(|e| {
                        FileShieldedStoreError::Storage(format!("read claim-key reservation: {e}"))
                    })?;
                let holder = <[u8; 16]>::try_from(holder.as_slice()).map_err(|_| {
                    FileShieldedStoreError::Storage(
                ...
                if holder == token.0 {
                    ...
                    .map_err(|e| {
                        FileShieldedStoreError::Storage(format!("re-stamp claim-key reservation: {e}"))
                    })?;
                    ClaimKeyReservation::Acquired
                } else {
            ...
            tx.commit().map_err(|e| {
                FileShieldedStoreError::Storage(format!("commit claim-key reservation: {e}"))
            })?;
            (reservation, pending)
        };
    ...
    fn arm_redrive_under_claim(
    ...
    ) -> Result<bool, Self::Error> {
        {
            ...
            let tx = conn
                .transaction_with_behavior(rusqlite::TransactionBehavior::Immediate)
                .map_err(|e| FileShieldedStoreError::Storage(format!("begin armed claim write: {e}")))?;
            ...
            let foreign_hold: i64 = tx
                ...
                .map_err(|e| FileShieldedStoreError::Storage(format!("read claim-key reservation: {e}")))?;
            if foreign_hold > 0 {
                return Ok(false);
            }
            let live: i64 = tx
                ...
                .map_err(|e| FileShieldedStoreError::Storage(format!("read claim lease: {e}")))?;
            if live == 0 {
            ...
            .map_err(|e| FileShieldedStoreError::Storage(format!("persist claim record: {e}")))?;
            ...
            .map_err(|e| FileShieldedStoreError::Storage(format!("restamp claim lease: {e}")))?;
            ...
            .map_err(|e| FileShieldedStoreError::Storage(format!("restamp claim-key reservation: {e}")))?;
            tx.commit()
                .map_err(|e| FileShieldedStoreError::Storage(format!("commit armed claim write: {e}")))?;
        }
        self.subwallets.entry(id).or_default().arm_redrive(redrive);
        Ok(true)
    }
    ...
    fn end_claim_admission(&mut self, token: AdmissionToken) -> Result<(), Self::Error> {
        ...
        let tx = conn
            .transaction_with_behavior(rusqlite::TransactionBehavior::Immediate)
            .map_err(|e| FileShieldedStoreError::Storage(format!("begin claim lease release: {e}")))?;
        tx.execute(
            "DELETE FROM shielded_lifecycle_admission WHERE token = ?1 AND destructive = 0",
            rusqlite::params![token.0.as_slice()],
        )
        .map_err(|e| FileShieldedStoreError::Storage(format!("release claim lease: {e}")))?;
        tx.execute(
            "DELETE FROM shielded_one_time_claim_reservation WHERE token = ?1",
            rusqlite::params![token.0.as_slice()],
        )
        .map_err(|e| FileShieldedStoreError::Storage(format!("release claim-key reservation: {e}")))?;
        tx.commit()
            .map_err(|e| FileShieldedStoreError::Storage(format!("commit claim lease release: {e}")))?;
        Ok(())
    }
    ...
    fn begin_destructive_admission(
        ...
        let tx = conn
            .transaction_with_behavior(rusqlite::TransactionBehavior::Immediate)
            .map_err(|e| FileShieldedStoreError::Storage(format!("begin destructive admission: {e}")))?;
        Self::reap_expired_admissions(&tx, now_ms)?;
        ...
        .map_err(|e| FileShieldedStoreError::Storage(format!("insert destructive barrier: {e}")))?;
        ...
        let live: i64 = tx
            ...
            .map_err(|e| FileShieldedStoreError::Storage(format!("count live claim leases: {e}")))?;
        tx.commit()
            .map_err(|e| FileShieldedStoreError::Storage(format!("commit destructive admission: {e}")))?;
    ...
    fn end_destructive_admission(&mut self, token: AdmissionToken) -> Result<(), Self::Error> {
        ...
        .map_err(|e| FileShieldedStoreError::Storage(format!("release destructive barrier: {e}")))?;
        Ok(())
    }
}
