...
impl NetworkShieldedCoordinator {
    ...
    async fn register_locked(
        ...
        // (#4313 review finding coordinator.rs:856). A purge that still
        // will not run is logged rather than raised — it must not fail the
        // registration — and the wallet's hydration flag stays cleared below,
        // forcing the bind to re-run its restore rather than trusting the
        // store.
