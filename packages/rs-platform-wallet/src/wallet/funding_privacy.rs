...
//! and `AssetLockManager::build_asset_lock_tx_from_selected_account` for the
...
//! that no public spend entry point crosses domains. See the `guardrail` module
//! docs for why the pair is sufficient.
...
mod guardrail {
    ...
    //!   enumerates the crate's public L1 spend entry points and proves, over a
    //!   wallet whose balance is split across two domains, that none of them
    //!   can fund a transaction neither domain covers alone. This catches a
    //!   semantic regression in an *existing* entry point even if it is written
    //!   without ever naming `all_funding_accounts` (e.g. by iterating the
    //!   account maps directly). Its blind spot: it only knows about the entry
    //!   points listed in it, so a *newly added* spend path is invisible to it.
    //!
    ...
    /// cross-domain union. Every entry point below must refuse it.
    const CROSS_DOMAIN_ONLY: u64 = 15_000_000;

    /// No public L1 spend entry point may fund a transaction that requires
    /// coins from more than one funding account.
    ///
    /// The fixture splits the balance evenly across two privacy domains (BIP44
    /// and DIP-9 CoinJoin) and asks each entry point for an amount that neither
    /// domain covers alone but their union does. A union-funding path succeeds
    /// here; a compliant single-domain path fails. Failing is the CORRECT
    /// outcome and matches the approved asset-lock contract: selection is
    /// confined to the named account, and a shortfall surfaces as a typed
    /// insufficient-funds error rather than silently reaching into a second
    /// domain.
    ///
    ...
    async fn no_spend_entry_point_unions_by_default() {
        ...
        // 2 & 3. The asset-lock entry point, shielded and non-shielded.
        ...
        for funding_type in [
            ...
            assert!(
                lock.is_err(),
                "{funding_type:?} asset-lock funding must not union BIP44 with \
                 CoinJoin, got {lock:?}"
            );
        }
    }
