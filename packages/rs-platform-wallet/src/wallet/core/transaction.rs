...
impl<B: TransactionBroadcaster + ?Sized> CoreWallet<B> {
    ...
    pub(crate) async fn release_transaction_reservation(
        ...
        // per account under the one `token`, so each must reconcile. A pooled
        // send spans BIP44 + BIP32 + any DashPay receiving accounts, so this is
        // the only shape that can release everything a pooled build reserved.
