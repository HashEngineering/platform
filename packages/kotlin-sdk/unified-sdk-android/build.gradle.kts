...
android {
    ...
    namespace = "org.dash.sdk.unified"
    compileSdk = 35

    defaultConfig {
        minSdk = 24

        // ABI filters - match what build_unified_android.sh produces.
    ...
    // Pre-built .so files from build_unified_android.sh are placed in src/main/jniLibs.
    // UNIFIED flavor ships only librs_unified_sdk_ffi.so.
    sourceSets {
        getByName("main") {
...
dependencies {
    // Unified flavor: full SDK + key-wallet + platform-wallet + shielded. Re-exports
    // core via :unified-sdk-jvm, whose dash-sdk-native.properties pins the native lib
    // to rs_unified_sdk_ffi.
    api(project(":unified-sdk-jvm"))
