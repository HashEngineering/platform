...
:: Build a flavor's FFI shared library for the local Windows machine (for JVM unit tests).
:: Usage: build_local.bat [platform^|unified]
::
::   platform (default)   builds rs-sdk-ffi         -> target\release\rs_sdk_ffi.dll
...
::   gradlew.bat :platform-sdk-jvm:test
::   gradlew.bat :platform-sdk-jvm:test -PdashNativeLib=rs_unified_sdk_ffi   (unified)
...
if "%1"=="unified" (
    echo   gradlew.bat :platform-sdk-jvm:test -PdashNativeLib=rs_unified_sdk_ffi
) else (
    echo   gradlew.bat :platform-sdk-jvm:test
)
